...
var _normalised_cumulative_weights: Array[float]
...
func _init(items: Array[Variant], weights: Array[int]):
	...
	for item in items.size():
		...
		self._normalised_cumulative_weights.append(self._total_weight)
	for item in items.size():
		self._normalised_cumulative_weights[item] /= self._total_weight
	self._normalised_cululative_weights.sort()
